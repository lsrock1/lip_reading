...
class LipRead(nn.Module):
    ...
    def attention(self, query, key, value):
        ...
        q = self.attn[0](query.unsqueeze(1)).view(bs, length, -1, 1)
        k = self.attn[1](key.unsqueeze(1)).view(bs, length, -1, 1).transpose(-2, -1)
        v = self.attn[2](value.unsqueeze(1)).view(bs, length, -1, 1)
